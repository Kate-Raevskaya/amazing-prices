...
class Test extends StageTest {
    ...
    tests = [this.node.execute(async () => {
        ...
        this.wrongStyleMsg = (id, style) => {
            return `${thePage} ${theElement} ${id}'s ${style} is different from the Figma designs or description.`;
        };
        ...
        this.page.execute(() => {
            ...
            if (this.elementPosition(this.header))
                return wrong(this.topBottomMsg(this.header, "top"));

            // check if header width is correct
            const correctWidth = document.body.getBoundingClientRect()["width"];
            if (this.exactElementWidth(this.header, correctWidth))
                return wrong(this.wrongWidthMsg(this.header));

            // check if main has correct position
            const headerBottom = document.querySelector(this.header).getBoundingClientRect()["bottom"];
            if (this.elementPosition(this.main, headerBottom, headerBottom + 100, 0, 0))
                return wrong(this.wrongPositionMsg(this.main));

            // check if main width is correct
            if (this.exactElementWidth(this.main, correctWidth))
                return wrong(this.wrongWidthMsg(this.main));

            // check if footer is at the bottom of the page
            const mainBottom = document.querySelector(this.main).getBoundingClientRect()["bottom"];
            if (this.elementPosition(this.footer, mainBottom, mainBottom + 200, 0, 0))
                return wrong(this.topBottomMsg(this.footer, "bottom"));

            // check if footer width is correct
            if (this.exactElementWidth(this.footer, correctWidth))
                return wrong(this.wrongWidthMsg(this.footer));
        ...
        this.page.execute(() => {
            ...
            // check if #nav-brand is on the left
            let id = "#nav-brand";
            const headerBottom = document.querySelector(this.header).getBoundingClientRect()["bottom"];

            if (this.elementPosition(id, 10, headerBottom / 2, 10, 50))
                return wrong(this.wrongPositionMsg(id));

            // check nav position
            if (this.elementPosition(this.nav, 10, headerBottom / 2, 500, 800))
                return wrong(this.wrongPositionMsg(this.nav));
        ...
        this.page.execute(() => {
            ...
            if (this.elementPositionCompareY("#nav-brand", this.nav + " a:nth-child(2)", false))
                return wrong(this.wrongPositionCompareYMsg("#nav-brand", this.nav));
